    ...
    <DashboardLayout title="Notifications">
      <div className="section-header" style={{ marginBottom:20 }}>
        <div>
          ...
          {unread > 0 && <div className="section-sub">{unread} unread</div>}
        </div>
      </div>
          ...
          {items.map(n => (
            <div key={n.id} className="glass" style={{
              ...
              <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                {!n.is_read && (
                  <button className="btn-icon" title="Mark as read" onClick={() => markRead(n.id)}><Check size={14}/></button>
                )}
                {n.complaint_id && (
                  <button className="btn-icon" title="View complaint" onClick={() => router.push(`/employee/complaints/${n.complaint_id}`)}><ExternalLink size={14}/></button>
                )}
              </div>
            </div>
          ))}
