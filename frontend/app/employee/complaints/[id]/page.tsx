...
import { getComplaint, withdrawComplaint, rateComplaint, updateComplaint } from '@/lib/api';
import { ArrowLeft, Star, Clock, Calendar, Brain, AlertTriangle, Edit3, X, Check, Paperclip } from 'lucide-react';
...
export default function EmployeeComplaintDetail() {
  ...
  const load = () => {
    setLoading(true);
    getComplaint(id).then(r => { setC(r.data); setEditForm({ title: r.data.title, description: r.data.description }); }).catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(load, [id]);

  const handleWithdraw = async () => {
    if (!confirm('Are you sure you want to withdraw this complaint?')) return;
    try { await withdrawComplaint(id); load(); setMsg('Complaint withdrawn.'); } catch { setMsg('Failed to withdraw.'); }
  };

  const handleRate = async () => {
    if (rating < 1) return;
    setRatingSubmitting(true);
    try { await rateComplaint(id, { rating, feedback: feedback || undefined }); load(); setMsg('Rating submitted!'); } catch { setMsg('Failed to rate.'); }
    finally { setRatingSubmitting(false); }
  };

  const handleEdit = async () => {
    setSaving(true);
    try { await updateComplaint(id, editForm); load(); setEditing(false); setMsg('Updated!'); } catch { setMsg('Failed to update.'); }
  ...
  return (
    <DashboardLayout title="Complaint Detail">
      <button className="btn btn-secondary" onClick={() => router.back()} style={{ marginBottom:20 }}><ArrowLeft size={14}/> Back</button>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
          <div style={{ flex:1, minWidth:0 }}>
            {editing ? (
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  <button className="btn btn-primary" onClick={handleEdit} disabled={saving}>{saving ? 'Saving...' : <><Check size={14}/> Save</>}</button>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              ...
              {c.primary_department && <span className="badge" style={{ background:'rgba(59,130,246,0.1)', color:'#60a5fa', border:'1px solid rgba(59,130,246,0.2)' }}>{c.primary_department}</span>}
            </div>
          </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
        ...
        <div className="glass" style={{ padding:20 }}>
          ...
          {[
            ...
            <div key={item.label} style={{ marginBottom:12 }}>
              <div style={{ fontSize:11, color:'#475569', textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:3 }}>{item.label}</div>
              <div style={{ fontSize:13, color:'#94a3b8', lineHeight:1.5 }}>{item.value}</div>
            </div>
          ))}
          {!c.ai_summary && <div style={{ fontSize:13, color:'#475569' }}>AI analysis pending...</div>}
        </div>
        ...
        <div className="glass" style={{ padding:20 }}>
          ...
          {[
            { label: 'Created', date: c.created_at, color: '#8b5cf6' },
          ...
          <div style={{ flex:1, minWidth:0 }}>
            {editing ? (
              ...
              <>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8, flexWrap:'wrap' }}>
                  <h2 style={{ fontSize:18, fontWeight:700, color:'#f1f5f9' }}>{c.title}</h2>
                  <span style={{ fontSize:11, color:'#6366f1', background:'rgba(99,102,241,0.1)', padding:'2px 8px', borderRadius:6 }}>#{c.id.slice(0,8)}</span>
                </div>
                <p style={{ fontSize:14, color:'#94a3b8', lineHeight:1.7, marginBottom:12 }}>{c.description}</p>
              </>
            )}
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              ...
              <PriorityBadge priority={c.priority_level}/>
              {c.primary_department && <span className="badge" style={{ background:'rgba(59,130,246,0.1)', color:'#60a5fa', border:'1px solid rgba(59,130,246,0.2)' }}>{c.primary_department}</span>}
              {c.is_anonymous && <span className="badge" style={{ background:'rgba(129,140,248,0.1)', color:'#818cf8', border:'1px solid rgba(129,140,248,0.2)' }}>Anonymous</span>}
            </div>
          </div>
          {c.status === 'PENDING' && !editing && (
            <div style={{ display:'flex', gap:8, flexShrink:0 }}>
